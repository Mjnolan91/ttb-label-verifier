...
- **Hermetic fixtures: the mock keys off the image FILENAME, not image bytes.** Fixtures live
  in `eval/fixtures/` (`cases.json` + mostly placeholder SVGs; ten rows are real rasters already
  in the repo — one supplied `.jpg`, the three `demo-*.png` labels generated by
  `scripts/generate-demo-labels.cjs`, the three `fear-the-dragon-*.jpg` files built by
  `scripts/make-fear-the-dragon-demo.cjs` (eval-only since 2026-06-11), and the three
  `fireball-*.jpg` demo files built by `scripts/make-fireball-demo.cjs` and byte-mirrored into
  `public/samples/`); the suite needs no real images. An
  ...
  filename alone.
- **Real images for the remaining placeholder scenarios can be supplied later.**
  `eval/fixtures/images/MANIFEST.md` is the row-by-row state: it lists the
