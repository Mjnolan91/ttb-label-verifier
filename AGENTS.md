...
- **Hermetic fixtures: the mock keys off the image FILENAME, not image bytes.** Fixtures live
  in `eval/fixtures/` (`cases.json` + mostly placeholder SVGs; four rows are real rasters already
  in the repo — one supplied `.jpg` and the three `demo-*.png` labels generated by
  `scripts/generate-demo-labels.cjs` and byte-mirrored into `public/samples/`); the suite needs no
  real images. An
