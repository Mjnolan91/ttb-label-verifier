...
- **Hermetic fixtures: the mock keys off the image FILENAME, not image bytes.** Fixtures live
  in `eval/fixtures/` (`cases.json` + mostly placeholder SVGs; seven rows are real rasters already
  in the repo — one supplied `.jpg`, the three `demo-*.png` labels generated by
  `scripts/generate-demo-labels.cjs`, and the three `fear-the-dragon-*.jpg` demo files built by
  `scripts/make-fear-the-dragon-demo.cjs` and byte-mirrored into `public/samples/`); the suite
  needs no real images. An
