...
the bytes). Rows #1–#7, #12–#16, and #20 are lightweight **`.svg` PLACEHOLDERS** that depict the
...
2026-06-11); **rows #17–#19 are the Fear the Dragon pair + its edited defect variant**
(real artwork `.jpg`, built by `node scripts/make-fear-the-dragon-demo.cjs` — eval fixtures only
since 2026-06-11, when the in-app demo switched to Fireball); **rows #21–#23 are the Fireball demo
pair + its edited defect variant** (real artwork `.jpg` from the public TTB COLA registry, built by
`node scripts/make-fireball-demo.cjs` and byte-mirrored into `public/samples/` for the verify
screen's "Load the sample label" buttons and the README's "Try it in two minutes" walkthrough), so
the live vision demo extracts genuine pixels. To exercise a real provider
...
> which is why the README's "Try it in two minutes" walkthrough points at the real Fireball rows
> #21–#23 (byte-mirrored into `public/samples/` for the one-click sample buttons; rows #8–#11 are
> real rasters too, eval-only since 2026-06-11), whose printed text matches their `extracted` block
> so the offline verdict equals a correct read.
...
| 9 | `demo-old-tom-clean.png` | `eval/fixtures/images/demo-old-tom-clean.png` | `demo-clean-approve` | **REAL generated raster** (720×820), eval fixture only since 2026-06-11 | Brand **OLD TOM DISTILLERY**; "Kentucky Straight Bourbon Whiskey"; **45% Alc./Vol. (90 Proof)**; **750 mL**; full canonical warning, ALL-CAPS **bold** `GOVERNMENT WARNING:` prefix. Everything correct. | **approve** |
| 10 | `demo-warning-title-case.png` | `eval/fixtures/images/demo-warning-title-case.png` | `demo-warning-title-case-reject` | **REAL generated raster** (720×820), eval fixture only since 2026-06-11 | Identical to #9 EXCEPT the warning prefix is rendered title-case **`Government Warning:`**. Sole defect. | **reject** |
| 11 | `demo-brand-typo.png` | `eval/fixtures/images/demo-brand-typo.png` | `demo-brand-typo-review` | **REAL generated raster** (720×820), eval fixture only since 2026-06-11 | Brand printed **`Old Tomm Distillery`** (one extra "m") vs claimed `Old Tom Distillery`. Sole near-miss; alcohol and warning correct. | **review** |
...
| 17 | `fear-the-dragon-front.jpg` | `eval/fixtures/images/fear-the-dragon-front.jpg` | `fear-the-dragon-front-panel-alone` | **REAL artwork** (1200x1200, Dragon Distillery x Flying Dog), eval-only since 2026-06-11 | Front panel of the demo PAIR: brand **Fear the Dragon**; "Spirit Distilled from Grain and Pumpkin"; statement of composition; **50% ALC./VOL. (100 PROOF)**; responsibility line "Distilled & Bottled by Dragon Distillery, LLC Frederick, MD". No net contents, no warning (both live on the back), so ALONE it must never approve; uploading front+back together on the verify screen yields the joint-read approve. | **review** |
| 18 | `fear-the-dragon-back.jpg` | `eval/fixtures/images/fear-the-dragon-back.jpg` | `fear-the-dragon-back-panel-alone` | **REAL artwork** (900x1126), eval-only since 2026-06-11 | Back panel of the demo pair: marketing text, **750ML**, and the full statutory warning printed ENTIRELY in caps with a **bold** `GOVERNMENT WARNING:` prefix (the body's case folds; only the prefix carries the caps+bold rule). The warning block is RE-RENDERED at print fidelity by the make-script (the 450px source's bold differential was honestly unverifiable, even by the live judge + focus escalation); the two back variants differ ONLY in the prefix weight. Alone it lacks class/alcohol/responsibility, so completeness routes it to review. | **review** |
| 19 | `fear-the-dragon-warning-not-bold-back.jpg` | `eval/fixtures/images/fear-the-dragon-warning-not-bold-back.jpg` | `fear-the-dragon-warning-not-bold-reject` | **EDITED TEST ARTIFACT** (built by `scripts/make-fear-the-dragon-demo.cjs`; the real label is compliant) | Identical to #18 EXCEPT the prefix prints **`Government Warning:` in title case and regular weight** (wording verbatim). Title case is deliberate: the caps rule is judged from the TRANSCRIPT so a live model hard-fails deterministically, while a weight-only defect measured live as "could not be verified as bold" and routed to review (the conservative tri-state working as designed). | **reject** |
| 20 | `jolly-jerrys-region-origin.svg` | `eval/fixtures/images/jolly-jerrys-region-origin.svg` | `region-origin-not-country-review` | Portrait label, ~600x800; clean | Brand **JOLLY JERRY'S** (Jolly Jerry's Distillery, Bridgetown, Barbados); class/type **Rum**; **40% ALC/VOL (80 PROOF)**; net **750 mL**; origin line **"IMPORTED FROM THE CARIBBEAN"**; full canonical warning, ALL-CAPS **bold** prefix. The origin statement names a REGION, not a country — it matches the application verbatim, and the defect is the statement itself (CBP marking requires the country, e.g. "Product of Barbados"). | **review** |
| 21 | `fireball-front.jpg` | `eval/fixtures/images/fireball-front.jpg` | `fireball-front-panel-alone` | **REAL artwork** (1104x1692, Fireball Cinnamon Whisky / Sazerac, from the public TTB COLA registry), the in-app demo sample + `public/samples/` | Front panel of the demo PAIR: brand **FIREBALL**; distinctive/fanciful name **CINNAMON WHISKY**; specialty designation/statement of composition **WHISKY WITH NATURAL CINNAMON FLAVOR**; **ALC. 33% BY VOL. (66 PROOF)**. No net contents, no warning, no responsibility line (all live on the back), so ALONE it must never approve; the verify screen's "Load the sample label" button places front+back together for the joint-read approve. | **review** |
| 22 | `fireball-back.jpg` | `eval/fixtures/images/fireball-back.jpg` | `fireball-back-panel-alone` | **REAL artwork** (1344x2048), the in-app demo sample + `public/samples/` | Back panel of the demo pair: marketing text, **750 ML**, importer line **"IMPORTED BY SAZERAC CO., INC., LOUISVILLE, KY"**, **"PRODUCT OF CANADA"** (the import path: origin inferred AND the country marking present), and the full statutory warning printed ENTIRELY in caps with a **bold** `GOVERNMENT WARNING:` prefix (the body's case folds; only the prefix carries the caps+bold rule). The warning block is RE-RENDERED at print fidelity by the make-script (the 672px source's bold differential is honestly unverifiable at web resolution); the two back variants differ ONLY in the prefix case+weight. Alone it lacks class/alcohol, so completeness routes it to review. | **review** |
| 23 | `fireball-warning-not-bold-back.jpg` | `eval/fixtures/images/fireball-warning-not-bold-back.jpg` | `fireball-warning-not-bold-reject` | **EDITED TEST ARTIFACT** (built by `scripts/make-fireball-demo.cjs`; the real label is compliant) | Identical to #22 EXCEPT the prefix prints **`Government Warning:` in title case and regular weight** (wording verbatim). Title case is deliberate: the caps rule is judged from the TRANSCRIPT so a live model hard-fails deterministically, while a weight-only defect measures as "could not be verified as bold" and routes to review (the conservative tri-state working as designed). | **reject** |
...
The `.svg` rows above (#1–#7, #12–#16, and #20) are **hermetic placeholders**: the mock provider keys off the
...
then only for whichever scenario you want to run live (rows #8–#11 and #17–#23 are already real
rasters, and the README's demo walkthrough points at the Fireball rows #21–#23).
